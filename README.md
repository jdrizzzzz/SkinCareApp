...
#### Unpushed/local files required to build

This repo expects a few files that are intentionally not committed. Create them locally before building:

- `.env` with your environment variables, for example:
  ```bash
  WEATHER_API_KEY=your_key_here
  ```
- Firebase config files generated by FlutterFire:
  - `android/app/google-services.json`
  - `ios/Runner/GoogleService-Info.plist`
  - `macos/Runner/GoogleService-Info.plist` (if building for macOS)

After adding those files, run the usual Flutter build/run commands.
